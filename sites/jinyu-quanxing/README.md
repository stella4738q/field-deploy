...
| 角色 | 慣例 IP | 說明 |
|---|---|---|
| ems（×2） | .101 / .102 | EMS 雙機 HA（keepalived VIP + haproxy → :8888） |
| data-collection | .103 | 資料收集機，兼 Samba storage（[storage] share，guest + force user） |
| database-server | .105 | MongoDB **原生安裝**（非 docker，見 `mongodb/README.md`） |

另外**每台都部署 NTP container**（chrony：校時 + 對場內設備供時）。

尚未建置。現場確認連線資訊後：

1. `hosts.conf` 取消 `qx-*` 註解並填入實際 IP/port
