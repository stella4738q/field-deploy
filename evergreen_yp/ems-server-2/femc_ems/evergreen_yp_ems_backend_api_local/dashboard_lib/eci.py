...
#         self.eci_list = None
#
#     def get_eci(self, eci_indicator_list=[]):
#         if len(eci_indicator_list) > 0:
#             col_list = [item.col_name for item in eci_indicator_list]
#             self.pd_data = self.pd_data[col_list]
#         else:
#             col_list = self.pd_data.columns.values.tolist()
#
#         mean_list = self.pd_data.mean()
#         std_list = self.pd_data.std()
#         # 取usl lsl T
#         if len(eci_indicator_list) == 0:
#             t_list = ((mean_list + 3 * std_list) - (mean_list - 3 * std_list)) / 2
#         else:
#             t_list = [(item.usl - item.lsl) / 2 if item.cal_type == 0 else
#                       ((mean_list[idx] + item.usl * std_list[idx]) - (
#                                   mean_list[idx] - item.lsl * std_list[idx])) / 2
#                       for idx, item in enumerate(eci_indicator_list)]
#         eci_list = []
#         ici_list = []
#         key = 0
#         for idx, data in self.pd_data.iterrows():
#             key = key + 1
#             ici = (np.array(data) - np.array(mean_list)) / np.array(t_list)
#             for element in range(len(ici)):
#                 if ici[element] > 1:
#                     ici[element] = 1
#                 if ici[element] < -1:
#                     ici[element] = -1
...
#             ici_append = [{'key': key, 'value': ici_dic[key]} for key in ici_dic.keys()]
#             ici_list.append({'key': key, 'value': ici_append[:40]})
#             eci_list.append({'key': key, 'value': eci})
#
