...
class FilesystemSettings:
    def __init__(self, base_path: str, target_name_list=None,
                 ...
                 file_extension: FileExtension = FileExtension.csv, meter_ms_folder_name: str = 'meter_ms'):
        ...
        self._meter_ms_folder_name = meter_ms_folder_name
    ...
    @property
    def meter_ms_folder_name(self):
        return self._meter_ms_folder_name

    @meter_ms_folder_name.setter
    def meter_ms_folder_name(self, value):
        self._meter_ms_folder_name = value
...
"""
USAGE:
settings = FilesystemSettings(target_name_list='*MVCB', target_data=FolderType.Current,
                                          current_data_folder_name='current_data',
                                          base_path=dao.base_path,
                                          file_extension=FileExtension.csv)
data = source.dao.read(criteria=settings)
"""


class FileSystemDao(AbstractGenericDao):
    @property
    def logger(self):
        return self._logger
    ...
    def __init__(self, config):
        self.config = \
            config[ConfigConstant.STORAGE.value][ConfigConstant.DATABASE.value][ConfigConstant.FILESYSTEM.value]

        setting_config = config[ConfigConstant.APP.value]
        log_path = setting_config[ConfigConstant.LOG_PATH.value]
        self.is_smb = False
        self.smb_conn = None
        self.smb_username = setting_config.get(ConfigConstant.SMB_USER.value, None)
        self.smb_password = setting_config.get(ConfigConstant.SMB_PWD.value, None)

        self.logger = Logger().create('FileSystemDao', level=logging.INFO, log_path=log_path)
        self.__init_base_path()

        if self.base_path.startswith('\\\\'):
            base_path = self.base_path.replace('\\\\', '')
            key = base_path.split('\\')
            self.smb_server = key[0]
            self.smb_service = key[1]
            self.base_path = str.join('\\', [p for p in key[2:]])
            self.is_smb = True

    def __init_base_path(self):
        base_path = self.config[ConfigConstant.BASE_PATH.value]
    ...
    def read(self, criteria: FilesystemSettings, **kwargs):
        ...
        if self.is_smb:
            self.smb_conn = SMBConnection(self.smb_username, self.smb_password, '', '')
            self.smb_conn.connect(self.smb_server, 445)
            folders = self.scan_smb_folder(self.smb_conn, criteria, criteria.base_path)
        else:
        ...
        for folder in folders:
            ...
            if self.is_smb:
                if folder.IsZip:
                    with self.smb_conn.retrieveFile(self.smb_service, folder.ZipFile, io.BytesIO()) as file_buffer:
                        with ZipFile(file_buffer, 'r') as archive:
                            ...
                            for file_name in criteria.target_name_list:
                                ...
                                for file in files:
                                    if criteria.file_extension.value == 'csv':
                                        try:
                                            df = pd.read_csv(
                                                archive.open(file), encoding="utf-8", on_bad_lines='skip',
                                                usecols=use_cols)
                                        except pd.errors.ParserError:
                                            df = pd.read_csv(
                                                archive.open(file), encoding="utf-8", on_bad_lines='skip',
                                                usecols=use_cols, engine='python')
                                    ...
                                    topic = arr_topic[0]
                                    if topic == 'rack':
                                        if arr_topic[1] == 'temp' or arr_topic[1] == 'volt':
                                            topic = f'{arr_topic[0]}_{arr_topic[1]}'
                    ...
                    files = self.smb_conn.listPath(self.smb_service, folder.Path)
                    for file in files:
                        file_name = file.filename
                        for patten in criteria.target_name_list:
                            if fnmatch.fnmatch(file_name, patten):
                                with io.BytesIO() as file_buffer:
                                    self.smb_conn.retrieveFile(
                                        self.smb_service, os.path.join(folder.Path, file_name), file_buffer)
                                    file_buffer.seek(0)

                                    if criteria.file_extension.value == 'csv':
                                        try:
                                            df = pd.read_csv(
                                                file_buffer, encoding="utf-8", on_bad_lines='skip', usecols=use_cols)
                                        except pd.errors.ParserError:
                                            df = pd.read_csv(
                                                file_buffer, encoding="utf-8", on_bad_lines='skip', usecols=use_cols,
                                                engine='python')
                                        ...
                                        df = pd.read_fwf(file_buffer)
                                    else:
                                        df = pd.DataFrame()

                                    arr_topic = os.path.basename(file_name).split('_')
                                    topic = arr_topic[0]
                                    if topic == 'rack':
                                        if arr_topic[1] == 'temp' or arr_topic[1] == 'volt':
                                            topic = f'{arr_topic[0]}_{arr_topic[1]}'
                ...
                if folder.IsZip:
                    ...
                    for file_name in criteria.target_name_list:
                        ...
                        for file in files:
                            if criteria.file_extension.value == 'csv':
                                try:
                                    df = pd.read_csv(file, encoding="utf-8", on_bad_lines='skip', usecols=use_cols)
                                except pd.errors.ParserError:
                                    df = pd.read_csv(
                                        file, encoding="utf-8", on_bad_lines='skip', usecols=use_cols, engine='python')
            ...
            if f.is_dir():
                if f.name == settings.current_data_folder_name:
                    if settings.target_data == FolderType.Current or settings.target_data is None:
                        ...
                        result.append(f_def)
                elif f.name == settings.meter_ms_folder_name:
                    if settings.target_data == FolderType.MeterMs or settings.target_data is None:
                        f_def = DirDefinition(FolderType.MeterMs, f.path)
                        result.append(f_def)
    ...
    def scan_smb_folder(self, conn: SMBConnection, settings: FilesystemSettings, base_path, level=0, result=None):
        if result is None:
            result = []
        for f in conn.listPath(self.smb_service, base_path):
            if f.filename in [".", ".."]:
                continue
            f.path = os.path.join(base_path, f.filename)
            if f.isDirectory:
                if f.filename == settings.current_data_folder_name:
                    if settings.target_data == FolderType.Current or settings.target_data is None:
                        f_def = DirDefinition(FolderType.Current, f.path)
                        result.append(f_def)
                elif settings.target_data == FolderType.History or settings.target_data is None:
                    self.scan_smb_folder(conn, settings, f.path, level + 1, result)
                    if level == 3:
                        f_def = DirDefinition(FolderType.History, f.path)
                        if settings.cal_type == CallType.NonDef:
                            result.append(f_def)
