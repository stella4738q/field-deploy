...
class PCSGetData(Resource):
    ...
    def get_col_by_brand(brand):
        ...
        if 'PCS_1' in brand:
            col = [
                'frequency', 'volt_rs', 'volt_st', 'volt_tr', 'volt_avg', 'current_r', 'current_s', 'current_t',
                'reactive_power', 'active_power', 'dc_active_power', 'battery_voltage'
            ]
        return col
    ...
    def get(self):
        ...
        try:
            ...
            pd_res = dict()
            columns = self.get_col_by_brand(equipment_id)
            if data_type == 'minute':
