...
class GETFemcmsgLog(Resource):
    logger = Logger().create('Advanced-tek API - Femc msg log', level=logging_level, log_path=log_path)
    dao = StorageFactory(config).get_dao_factory().get_dao()

    @token_required(token_module)
    def get(self):
        self.logger.info('get Femc msg log')
        pd_res = []
        try:
            ...
            pd_log = self.dao.get_femc_msg_log(
                sdate=start_date, edate=end_date, notify_rule=rule_id, notify_token=token_id)
            if not pd_log.empty:
                pd_notify_token = self.dao.get_femc_msg_token(_id=token_id)
                pd_notify_rule = self.dao.get_femc_msg_rule(_id=rule_id)
