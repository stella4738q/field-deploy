...
class GETFemcmsgSetting(Resource):
    logger = Logger().create('Evergreen API - Femc msg setting', level=logging_level, log_path=log_path)
    dao = StorageFactory(config).get_dao_factory().get_dao()
    collection = 'notify setting'

    @token_required(token_module)
    def get(self):
        self.logger.info('[get] Femc msg setting')
        pd_res = []
        try:
            ...
            if not user_info.is_admin:
                ...
                if (len(user_info.user_field_permission) == 0 or field not in allow_field or
                        not user_info.user_field_permission[field][
                                PermissionConst.FEMC_NOTIFY_SETTING_GET.value]):
                    raise Exception('You dont have permission to access.')
            try:
                pd_notify_token = self.dao.get_femc_msg_token(_id=notify_token)
                pd_notify_token = pd_notify_token.rename(columns={'_id': ApiConst.NOTIFY_TOKEN.value})

                pd_notify_setting = self.dao.get_femc_msg_setting(notify_token=notify_token)
                ...
                pd_notify_rule = self.dao.get_femc_msg_rule(_id=list(notify_rule_ids))
    ...
    @token_required(token_module)
    def post(self):
        logger = Logger().create('Advanced-tek API - post Femc msg setting', level=logging_level, log_path=log_path)
        logger.info('[post] Femc msg')
        pd_res = []
        try:
            ...
            if data_list:
                self.logger.info('deleted Femc msg setting')
                pd_notify_setting = self.dao.get_femc_msg_setting(notify_token=notify_token)
                notify_setting_ids = pd_notify_setting['_id'].tolist() if not pd_notify_setting.empty else []
                pd_notify_setting = self.dao.delete_femc_msg_setting(notify_setting_ids)

                self.logger.info('create Femc msg setting')
                for data in data_list:
                    notify_rule = data.get("notify_rule")
                    notify_token = data.get("notify_token")
                    pd_notify_setting = self.dao.create_femc_msg_setting(notify_rule=notify_rule, notify_token=notify_token)
