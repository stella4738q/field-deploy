    ...
    <div class="row">
      ...
      <div class="col-8">
        ...
        <el-table :data="PageController.CurrentData" size="small" stripe border class="rounded-3"
          ...
          <el-table-column label="月份" prop="date" align="center">
            <template #default="scope">
              {{ scope.row.date ? moment(scope.row.date).format('YYYY-MM') : '' }}
            </template>
          </el-table-column>
          <el-table-column label="持續時間" prop="duration" align="center"></el-table-column>
      ...
      <div class="col-4">
        <div id="ReportChartMonth" style="height:400px"></div>
      </div>
    </div>
...
export default {
  ...
  beforeUnmount() {
    if (ReportChartMonth) ReportChartMonth.dispose();
    ReportChartMonth = null;
  },
  created() {
  },
  watch: {
  ...
  methods: {
    getData() {
      ...
      // vm.PageController.DataSource = reportDataData.data[0].raw_data;
      // vm.PlotData = reportDataData.data[0].plot_data;
    ...
    ChartIni() {
      var option = {
        ...
        xAxis: [
          {
            type: 'category',
            data: this.PlotData.date && this.PlotData.date.length > 0 ? this.PlotData.date.map(value => moment(value).format('YYYY-MM')) : [],
          }
        ],
        yAxis: [
          {
      ...
      this.$nextTick(function () {
        if (ReportChartMonth == null) {
          ReportChartMonth = echarts.init(document.getElementById("ReportChartMonth"));
        }
        ReportChartMonth.setOption(option);
      });
    },
  },
};
