...
class CustomProtectSetting(Resource):
    ...
    def get(self):
        self.logger.info('[get] custom protect setting')
        pd_res = []
        try:

            _id = request.args.get(ApiConst.ID_.value, None)
            key = request.args.get(ApiConst.KEY.value, None)
            value = request.args.get(ApiConst.VALUE.value, None)

            # get data
            pd_data = self.dao.get_custom_protect(_id=_id, key=key, value=value)
    ...
    @token_required(token_module)
    def post(self):
        self.logger.info('[post]  custom protect setting')
        pd_res = []
        try:
            # check permission
            ...
            for data in data_list:
                ...
                self.logger.info('update custom protect data')
                self.dao.update_custom_protect(_id, value)
