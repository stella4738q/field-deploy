...
class GETFemcmsgToken(Resource):
    logger = Logger().create('Evergreen API - Femc msg token', level=logging_level, log_path=log_path)
    dao = StorageFactory(config).get_dao_factory().get_dao()

    @token_required(token_module)
    def get(self):
        self.logger.info('[get] Femc msg token')
        pd_res = []
        ...
        try:
            ...
            if len(user_info.user_field_permission) > 0 and field not in allow_field or \
                    not user_info.user_field_permission[field][
                        PermissionConst.FEMC_NOTIFY_TOKEN_GET.value]:
            ...
            pd_data = self.dao.get_femc_msg_token(_id=_id, name=name, token=token)
    ...
    @token_required(token_module)
    def post(self):
        self.logger.info('[post] Femc msg token')
        pd_res = []
        ...
        try:
            ...
            pd_notify_token = self.dao.get_femc_msg_token(_id=_id)
            ...
            if _id in pd_notify_token:
                ...
                if len(user_info.user_field_permission) > 0 and field not in allow_field or \
                        not user_info.user_field_permission[field][
                            PermissionConst.FEMC_NOTIFY_TOKEN_POST_UPDATE.value]:
                    raise Exception('You dont have permission to access.')

                self.logger.info('update notify_token')
                pd_data = self.dao.update_femc_msg_token(_id=_id, name=name, token=token)
            else:
                # check permission
                self.logger.info('check permission')
                user_info: UserData = session[token_module.auth_session_key]
                ...
                if len(user_info.user_field_permission) > 0 and field not in allow_field or \
                        not user_info.user_field_permission[field][
                            PermissionConst.FEMC_NOTIFY_TOKEN_POST_CREATE.value]:
                    raise Exception('You dont have permission to access.')

                self.logger.info('create notify_token')
                pd_data = self.dao.create_femc_msg_token(_id=_id, name=name, token=token)
    ...
    @token_required(token_module)
    def delete(self):
        self.logger.info('[delete] Femc msg token')
        user_info: UserData = session[token_module.auth_session_key]
        ...
        try:
            ...
            if len(user_info.user_field_permission) > 0 and field not in allow_field or \
                    not user_info.user_field_permission[field][
                        PermissionConst.FEMC_NOTIFY_TOKEN_DELETE.value]:
                raise Exception('You dont have permission to access.')

            _id = request.get_json()[ApiConst.ID_.value]
            self.dao.delete_femc_msg_token([_id])
            message = SUCCESS_MESSAGE
        except Exception as e:
            message = repr(e)
