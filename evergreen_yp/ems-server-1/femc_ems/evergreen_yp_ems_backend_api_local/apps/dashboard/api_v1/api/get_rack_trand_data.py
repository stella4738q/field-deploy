...
class GetRackTrandData(Resource):
    @staticmethod
        ...
        try:
            ...
            if len(user_info.user_field_permission) > 0 and field not in allow_field or \
                    not user_info.user_field_permission[field][
                        PermissionConst.BMS_GET_DATA.value]:
            ...
            if bmu_data.empty:
                ...
                bmu_data = bmu_data[
                    ...
                    (pd.to_datetime(bmu_data['data_time']) <= e_date)
                ]
                if bmu_data.empty:
                    raise Exception('找不到相關檔案。')
            ...
            # 時間轉換
            bmu_data['data_time'] = pd.to_datetime(bmu_data['data_time'], format='%Y-%m-%d %H:%M:%S.%f')
            ...
            message = str(e)
        return {'message': message, 'data': pd_dict}, 200, None
