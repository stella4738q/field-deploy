...
class GETFemcmsgRule(Resource):
    logger = Logger().create('Evergreen API - Femc msg rule', level=logging_level, log_path=log_path)
    ...
    def get(self):
        ...
        """
        logger = Logger().create('Advanced-tek API - get Femc msg rule', level=logging_level, log_path=log_path)
        logger.info('[get] Femc msg rule')
        pd_res = []
        ...
        try:
            ...
            pd_data = self.dao.get_femc_msg_rule(_id=_id, code=code, name=name, description=description)
    ...
    @token_required(token_module)
    def post(self):
        self.logger.info('[post] Femc msg')
            ...
            if _id:
                ...
                if len(user_info.user_field_permission) > 0 and field not in allow_field or \
                        not user_info.user_field_permission[field][
                            PermissionConst.FEMC_NOTIFY_RULE_POST_UPDATE.value]:
                    raise Exception('You dont have permission to access.')

                self.logger.info('update Femc msg rule')
                pd_data = self.dao.update_femc_msg_rule(_id=_id, code=code, name=name, description=description)
            else:
                # check permission
                self.logger.info('check permission')
                user_info: UserData = session[token_module.auth_session_key]
                ...
                if len(user_info.user_field_permission) > 0 and field not in allow_field or \
                        not user_info.user_field_permission[field][
                            PermissionConst.FEMC_NOTIFY_RULE_POST_CREATE.value]:
                    raise Exception('You dont have permission to access.')

                # check code沒有存在才新增
                pd_data = self.dao.get_femc_msg_rule(code=code)
                if not pd_data.empty:
                    raise ValueError('Code exists.')

                self.logger.info('create femc_msg_rule')
                pd_data = self.dao.create_femc_msg_rule(_id=_id, code=code, name=name, description=description)
    ...
    def delete(self):
        ...
        try:
            ...
            if len(user_info.user_field_permission) > 0 and field not in allow_field or \
                    not user_info.user_field_permission[field][
                        PermissionConst.FEMC_NOTIFY_RULE_DELETE.value]:
                raise Exception('You dont have permission to access.')

            self.dao.delete_femc_msg_rule([_id])
            message = SUCCESS_MESSAGE
