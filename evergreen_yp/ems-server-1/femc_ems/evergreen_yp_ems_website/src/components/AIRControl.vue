  ...
  <div class="row text-start g-3">
    <div class="col-12 col-md-4">
      <div class="card border-success p-2">
        ...
        <p class="fs-5 m-0 text-end mt-1">{{ SocketData?.air_status ? '開啟' : '關閉' }}</p>
      </div>
    </div>
  ...
  <table class="table w-100 table-bordered text-start align-middle mt-2">
    <tbody>
      <tr>
        ...
        <td colspan="3">
          <el-button-group>
            <el-button type="primary" :disabled="(SocketData?.air_status) || !ControlPriviage"
              @click="AIR_ON_OFF(SocketData.equipment_id, true)">開</el-button>
            <el-button type="danger" :disabled="!(SocketData?.air_status) || !ControlPriviage"
...
export default {
  ...
  computed: {
    ...
    ControlPriviage: function () { return this.$store.getters.userPriviagePage('P01-04') && this.OPERATE_MODE !== 1; },
  },
  mounted() {
  },
  created() {
