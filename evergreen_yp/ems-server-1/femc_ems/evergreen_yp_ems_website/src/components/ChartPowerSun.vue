...
export default {
  ...
  methods: {
    ...
    updateChart() {
      ...
      let solarUpdated = false;

      this.LineChartDataPowerSun.forEach(dcu => {
        dcu.inverter?.forEach((inv, idx) => {
          const inverterIndex = idx + 1;
          const newDataTime = inv.data_time?.split('.')[0];
          const lastData = this.chartData[`INVERTER_${inverterIndex}`]?.slice(-1)[0];

          if (!lastData || lastData[0] !== newDataTime) {
            this.chartData[`INVERTER_${inverterIndex}`].push([
              newDataTime,
              parseFloat(this.formatNumber(inv.dc_k_watt)).toFixed(3)
            ]);

            if (!solarUpdated && this.LineChartDataSolar.length > 0) {
    ...
    async initHistory() {
      ...
      try {
        ...
        if (response.data.Success) {
          ...
          if (hisData) {
            // 載入 inverter 資料
            const inverterKeys = ['INVERTER_1', 'INVERTER_2', 'INVERTER_3'];
            inverterKeys.forEach(key => {
              if (hisData[key]) {
                vm.chartData[key] = [...hisData[key]];
              }
            });

            // 統一時間軸（只要任一 inverter 有該時間就加入）
            const timeSet = new Set();
            inverterKeys.forEach(key => {
              hisData[key]?.forEach(([time]) => timeSet.add(time));
            });

            // 過濾 solar_exposure，只留下有對應時間點的資料
            if (hisData['solar_exposure']) {
              vm.chartData['solar_exposure'] = hisData['solar_exposure'].filter(
                ([time]) => timeSet.has(time)
              );
            }
          }
        } else {
