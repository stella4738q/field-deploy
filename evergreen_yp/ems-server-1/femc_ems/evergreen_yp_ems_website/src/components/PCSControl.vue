<template>
  <div class="row text-start g-3">
    <div class="col-12 col-md-6">
      <div class="card border-success p-2">
        <p class="fs-6 m-0 border-bottom border-success">狀態</p>
  ...
  <table class="table w-100 table-bordered text-start align-middle mt-2">
    <tbody>
      ...
      <tr v-if="$store.getters.userPriviagePage('P01-08')">
        ...
        <td colspan="3">
          <el-button-group>
            ...
            <el-button type="success" :disabled="!(myPCS?.CanBSOff == true) || !ControlPriviage"
              @click="PCS_BLACK_START_FAULT_RESET(myPCS.equipment_id)">故障清除</el-button>
          </el-button-group>
        </td>
      </tr>
      <tr>
        ...
        <td>
          <div class="input-group input-group-sm ">
            ...
            <button class="btn btn-danger" type="button" :disabled="!(myPCS?.CanPower == true) || !ControlPriviage"
              @click="PCS_SET_POWER_2(allPCS, power1, 1)">自動分配</button>
          </div>
          <div class="input-group input-group-sm mt-2">
            ...
            <button class="btn btn-danger" type="button" :disabled="!(myPCS?.CanPower == true) || !ControlPriviage"
              @click="PCS_SET_POWER_2(allPCS, power2, -1)">自動分配</button>
          </div>
...
export default {
  ...
  computed: {
    ...
    ...mapState(["SocketDataALL", "PCS1", "PCS2", "OPERATE_MODE"]),
    myPCS: function () {
  ...
  methods: {
    ...
    PCS_ON_OFF(id, value) {
      ...
      vm.$confirm(`${this.PCSStoreID} 確定執行【${ value ? '開機': '關機' }】指令？`, '提示', {
        confirmButtonText: '確定', cancelButtonText: '取消', type: 'warning'
      }).then(() => {
    ...
    PCS_SET_POWER(id, value, ab) {
      ...
      vm.$confirm(`${id} 確定執行【${ab === 1 ? `放電: ${Math.abs(value)}` : `充電: ${Math.abs(value)}`}kW】指令？`, '提示', {
        confirmButtonText: '確定', cancelButtonText: '取消', type: 'warning'
      }).then(() => {
        vm.$loading();
        var postData = {
    ...
    PCS_SET_POWER_2(pcs_list, value, ab) {
      const vm = this;
      vm.$confirm(`確定執行【${ab === 1 ? `放電: ${Math.abs(value)}` : `充電: ${Math.abs(value)}`}kW】自動分配指令？`, '提示', {
        confirmButtonText: '確定', cancelButtonText: '取消', type: 'warning'
      }).then(() => {
        vm.$loading();

        const pcsCount = pcs_list.length;
        const base = value >= 0
          ? Math.floor(value / pcsCount)
          : Math.ceil(value / pcsCount);

        let remainder = value - base * pcsCount;

        pcs_list.forEach((x) => {
          let power = base;

          if (remainder !== 0) {
            power += remainder > 0 ? 1 : -1;
    ...
    PCS_FAULT_RESET(id, value) {
      ...
      vm.$confirm(`${this.PCSStoreID} 確定執行【故障清除】指令?`, '提示', {
        confirmButtonText: '確定', cancelButtonText: '取消', type: 'warning'
      }).then(() => {
        vm.$loading();
        var postData = {
    ...
    PCS_BLACK_START_ON_OFF(id, value) {
      ...
      vm.$confirm(`${this.PCSStoreID} 確定執行【全黑啟動】【${ value ? '開啟': '關閉' }】指令？`, '提示', {
        confirmButtonText: '確定', cancelButtonText: '取消', type: 'warning'
      }).then(() => {
        vm.$loading();
        var postData = {
    ...
    PCS_BLACK_START_FAULT_RESET(id) {
      const vm = this;

      vm.$confirm('確定執行【全黑啟動-故障清除】指令?', '提示', {
        confirmButtonText: '確定', cancelButtonText: '取消', type: 'warning'
      }).then(() => {
        ...
        var postData = {
          customer: "advanced",
          code: "PCS",
          action: "PCS_BLACK_START_FAULT_RESET",
          equipment_id: id,
          value: true,
        };
        postModbusControl(postData).then(response => {
