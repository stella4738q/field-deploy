  ...
  <div class="row">
    <div class="col-12 mb-3">
      ...
      <ul class="chartT_list">
        ...
        <li>最高溫度電芯位址： {{ option.maxPostion }}</li>
        <li>最低溫度：{{ option.minVal?.toFixed(0) }} ℃</li>
        <li>最低電芯位址：{{ option.minPostion }}</li>
...
export default {
    ...
    option: function () {
      ...
      if (packObj) {
        packObj?.forEach((x, ind) => {
          ...
          let tempObj = x.cell_temp || x.cell_temperature;
  ...
  data() {
    return {
      chartId: "",
      TempValue: [
        this.RackData?.rack_max_cell_temperature ? this.RackData.rack_max_cell_temperature - 4 : 20,
        this.RackData?.rack_max_cell_temperature ?? 45
      ],
      RangeMax: 50,
      RangeMin: 0,
