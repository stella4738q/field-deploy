<template>
    <div class="row text-start g-3">
      <div class="col-12 col-md-6">
        <div class="card border-success p-2">
          ...
          <p class="fs-5 m-0 text-end mt-1">{{ SocketData?.status }}</p>
        </div>
      </div>
  
      <div class="col-12 col-md-6">
      <div class="card border-success p-2">
        <p class="fs-6 m-0 border-bottom border-success">DC狀態</p>
        <p class="fs-5 m-0 text-end mt-1">{{ myBMS.switch_status == 1 ? "開啟": "關閉" }}</p>
      </div>
    </div>
      <!-- <div class="col-12 col-md-6">
        <div class="card border-success p-2">
          <p class="fs-6 m-0 border-bottom border-success">RACK狀態</p>
          <div class="fs-5 p-0 text-end row m-0 text-white" v-if="myBMS.rack">
            ...
            <div class="col p-0 m-0 text-center border border-white mt-1" v-for="(rack, ind) in myBMS.rack.sort((a,b) => {return a.rack_no > b.rack_no ? 1 : -1})">
              <div :class="getRackClass(SocketData.bms.rack.equipment_id)"> {{ SocketData.bms.rack.rack_no }} </div>
            </div>
          </div>
        </div>
      </div> -->
    ...
    <tbody>
      <tr>
        <td style="width:80px">DC開關</td>
        <td>
          <el-button-group>
            <el-button type="primary" @click="BMS_DC_SWITCH_ONOFF(myBMS.equipment_id, true)" :disabled="!(myBMS?.switch_status == 0) || !ControlPriviage">開</el-button>
            <el-button type="danger" @click="BMS_DC_SWITCH_ONOFF(myBMS.equipment_id, false)" :disabled="!(myBMS?.switch_status == 1) || !ControlPriviage">關</el-button>
          </el-button-group>
        </td>
      </tr>
      <tr>
  ...
  export default {
    ...
    computed: {
      ...mapState(["SocketDataALL", "BMS1", "OPERATE_MODE"]),
