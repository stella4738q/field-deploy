  ...
  <div class="card border-0 shadow-sm">
    ...
    <div class="d-flex align-items-center mb-3">
      <el-form :model="Form" ref="Form" :rules="FormRule" :inline="true" label-position="top" class="align-items-end">
        ...
        <el-form-item>
          <el-button type="primary" class="me-2" :disabled="sendCheck" @click="SearchClick"><font-awesome-icon icon="search" /></el-button>
          <el-button type="primary" :disabled="sendCheck" @click="DownloadClick"><font-awesome-icon icon="download" /></el-button>
        </el-form-item>
      </el-form>
    </div>
    <!------------------------------------------------------------------------------------------->
    <el-tabs type="border-card" v-model="activeName">
        ...
        <div>
          <el-form-item class="mb-0" label="數據類型1" prop="type">
            <el-select v-model="OptionValue.Chart1" placeholder="請選擇" @change="setMeterChart1">
              <el-option v-for="option in enumlist.MeterAttrOption" :label="option.label" :value="option.val" />
            </el-select>
          </el-form-item>
      ...
      <el-tab-pane label="下載結果" name="tab2">
        <DataExportQueryMeter class="mt-2 mb-2" ref="dataExportQuery" equipment_id="METER"></DataExportQueryMeter>
      </el-tab-pane>
    </el-tabs>
    <!------------------------------------------------------------------------------------------->
  </div>
...
import { getMeterGetData , postExportAsyncData, getExportAsyncData } from '@/api/Api.js';
import enumlist from '@/JSON/EnumList.json';
import libDownload from "@/lib/libDownload.js";
import DataExportQueryMeter from '@/components/DataExportQueryMeter.vue';
...
export default {
  ...
  methods: {
    getDownList() {
      var vm = this;
      // vm.$loading();
      getExportAsyncData({ equipment_id: 'METER' })
        .then(response => {
      ...
      vm.$loading();
          postExportAsyncData(postData).then(response => {
            if(response.headers['content-type'] !== 'application/json'){
                ...
                let fileName = `電錶記錄_${vm.Form.equipment_id}_${vm.moment(sdate, 'YYYYMMDDHHmmss').format('YYYYMMDDHH')}-${vm.moment(edate, 'YYYYMMDDHHmmss').format('YYYYMMDDHH')}.csv`;
