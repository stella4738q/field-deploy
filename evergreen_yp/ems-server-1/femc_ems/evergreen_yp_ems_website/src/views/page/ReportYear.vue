    ...
    <div class="row">
      <div class="col-8">
        ...
        <el-table :data="PageController.CurrentData" size="small" stripe border class="rounded-3"
          ...
          <el-table-column label="年份" prop="date" align="center">
            <template #default="scope">
              {{ scope.row.date ? moment(scope.row.date).format('YYYY') : '' }}
            </template>
          </el-table-column>
      ...
      <div class="col-4">
        <div id="ReportChartYear" style="height:400px"></div>
      </div>
    </div>
...
export default {
  ...
  beforeUnmount() {
    if (ReportChartYear) ReportChartYear.dispose();
    ReportChartYear = null;
  },
  created() {
  },
  watch: {
    ...
    ChartIni() {
      var option = {
        ...
        xAxis: [
          {
            type: 'category',
            data: this.PlotData.date && this.PlotData.date.length > 0 ? this.PlotData.date.map(value => moment(value).format('YYYY')) : [],
          }
        ],
        yAxis: [
          {
      ...
      this.$nextTick(function () {
        if (ReportChartYear == null) {
          ReportChartYear = echarts.init(document.getElementById("ReportChartYear"));
        }
        ReportChartYear.setOption(option);
      });
    },
  },
};
