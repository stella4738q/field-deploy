<template>
  <div class="p-3 card border-0 shadow-sm">
    <div class="card-body">
      ...
      <div class="d-flex align-items-center mb-3">
        ...
        <el-form :model="Form" ref="Form" :rules="FormRule" label-width="100px">
          <el-form-item label="PCS" prop="pcs">
            <el-select v-model="Form.pcs" placeholder="請選擇...">
              <el-option value="PCS_1" label="PCS 1"> </el-option>
            </el-select>
          </el-form-item>
    ...
    <DataExportQuery ref="dataExportQuery" equipment_id="PCS_1"></DataExportQuery>
  </div>
</template>
<script>
...
import { getPCSGetDataExport, postExportAsyncData } from '@/api/Api.js';
...
export default {
  ...
  data() {
    ...
    return {
      ...
      FormRule: {
        pcs: [{ type: 'string', required: true, message: ' ', trigger: 'blur' }],
        ...
        if (valid) {
          ...
          postExportAsyncData(postData).then(response => {
            if(response.headers['content-type'] !== 'application/json'){
                ...
                let fileName = `PCS紀錄_${vm.moment(sdate, 'YYYYMMDDHHmmss').format("YYMMDDHH")}-${vm.moment(edate, 'YYYYMMDDHHmmss').format("YYMMDDHH")}.csv`;
          ...
          // getPCSGetDataExport(postData).then(response => {
          //   if(response.headers['content-type'] !== 'application/json'){
          //     var sdate = vm.Form.start_date + vm.Form.start_time.replace(':', '') + "00";
          //     var edate = vm.Form.end_date + vm.Form.end_time.replace(':', '') + "59";
          //     let fileName = `PCS紀錄_${vm.moment(sdate, 'YYYYMMDDHHmmss').format("YYMMDDHH")}-${vm.moment(edate, 'YYYYMMDDHHmmss').format("YYMMDDHH")}.csv`;
