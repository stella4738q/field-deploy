...
<script>
import { getFemcNotifyToken, delFemcNotifyToken, postFemcNotifyToken } from "@/api/Api.js";
  ...
  methods: {
    Delete(item) {
      ...
      vm.$confirm("確定刪除", "確認", {
        ...
        .then(() => {
          delFemcNotifyToken({ _id: item._id })
            .then((resopnse) => {
    ...
    getData() {
      const vm = this;
      vm.$loading();
      getFemcNotifyToken(vm.FormQuery)
        .then((resopnse) => {
          if (resopnse.data.message == "Success") {
